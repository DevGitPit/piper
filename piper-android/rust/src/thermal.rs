...
impl CpuTopology {
    ...
    fn parse_topology(
        ...
        let max_freq = cores_with_freq[0].1;
        let min_freq = cores_with_freq.last().map(|c| c.1).unwrap_or(max_freq);
...
impl UnifiedThermalManager {
    ...
    pub fn update(&mut self, buffer_seconds: f32, current_rtf: f32) -> ThermalMode {
        ...
        if self.baseline_rtf.is_none() && self.recent_rtf.len() == 5 {
            let avg = self.recent_rtf.iter().sum::<f32>() / 5.0;
            self.baseline_rtf = Some(avg);
            log::info!("Baseline RTF established: {:.2}x", avg);
        }
